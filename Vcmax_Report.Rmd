...
merged <- read.csv("Merged_data_to_analyze.csv")
library(ggplot2)
ggplot(merged, aes(Date.x, Vcmax, colour=Genotype)) + geom_smooth(aes(group=Genotype))+
...
merged <- read.csv("Merged_data_to_analyze.csv")
library(ggplot2)
ggplot(merged, aes(Date.x, Jmax, colour=Genotype)) + geom_smooth(aes(group=Genotype))+
        theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
